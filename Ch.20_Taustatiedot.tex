...
\section{Symmetrinen salaus}
 ...
 \emph{Groverin algoritmi} on kvanttialgoritmi, joka tuo neliöllisen nopeutuksen kvanttitietokoneella tehtyihin brute-force hyökkäyksiin. Kvanttitietokone joka käyttää Groverin algoritmia \emph{n}-bittisen salausavaimen purkamiseen laskee avaimen nopeudella $\sqrt{2^{n}} = 2^{n/2}$. Toisaalta näitä hyökkäyksiä voidaan hidastaa esimerkiksi tuplaamalla salausavaimen käyttämä bittien määrä. Tällöin 128-bittinen AES salaus antaisi 64-bittisen salauksen. AES-128 ja AES-256 ovat kvanttiturvallisia lohkosalaukseen perustuvia salausjärjestelmiä, joita pidetään kvanttiturvallisina. Nykypäivänä 80-bittistä salausta pidetään turvallisena rajana klassisella tietokoneella.
