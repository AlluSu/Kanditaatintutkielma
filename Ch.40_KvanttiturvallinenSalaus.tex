...
Käsittelemme kvanttiturvallisista salausmenetelmistä niiden idean sekä yleisiä ominaisuuksia muun muassa suorituskyvyssä ja julkisen avaimen koossa. Näitä eri kvanttiturvallisia salausmenetelmiä käyttäviä salausjärjestelmiä on implementoitu useita erilaisia variantteja. Näiden eri varianttien välillä on eroja esimerkiksi suorituskyvyssä ja julkisen avaimen koossa. Kuitenkin kaikkien niiden toiminta pohjautuu tässä esitellyihin ideoihin.
...
Koodipohjainen salaus (\emph{code-based cryptography}) on kvanttiturvallinen salauksen muoto, joka perustuu koodausteoriaan (\emph{coding theory}) (\cite{repka2014overview}). Robert McEliece esitteli vuonna 1978 koodipohjaisen julkisen avaimen salausjärjestelmän nimeltään \emph{MECS} (McEliece Cryptosystem). MECS on saanut viime vuosina enemmän huomiota kun kvanttiturvallisesta salauksesta on tullut ajankohtaisempaa.

MECS hyödyntää salauksessaan lineaarisia virheenkorjauskoodeja (\emph{error correcting codes}), tarkennettuna binäärisiä Goppa-koodeja (\cite{8012331}). MECS:issä salaisena avaimena toimii satunnainen binäärinen Goppa-koodi ja julkisena avaimena toimii satunnainen kehitinmatriisi (\emph{Generator matrix}), joka on siis jokin satunnainen permutaatio salaisena avaimena toimivasta binäärisestä Goppa-koodista (\cite{overbeck2009code}). 
